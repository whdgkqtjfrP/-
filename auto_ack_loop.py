...
print("비정상 ACK 자동 생성 및 수집 시스템 시작됨")

while True:
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    pcap_file = f"ackflood_{timestamp}.pcap"

    print(f"[{timestamp}] 비정상 ACK 트래픽 생성 시작")
    subprocess.Popen(
        ["hping3", "-A", TARGET_IP, "-p", "80", "-c", str(PACKET_COUNT)],
    ...
    subprocess.run(
        ["timeout", str(INTERVAL), "tcpdump", "-i", "any", "-w", pcap_file,
         "tcp[tcpflags] & tcp-ack != 0", "and", "src", "host", TARGET_IP],
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL
    )
